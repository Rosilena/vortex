...
uint32_t size = 4; //dimensione totale
uint32_t grid_dim = 2; //numero di blocchi nella griglia
uint32_t block_dim = 3; //numero di thread in un blocco
...
int run_kernel_test(const kernel_arg_t& kernel_arg) {
  ...
  for(uint32_t i = 0; i < size; i++ ){
    if(i % 2 == 0){
        std::cout << "Thread: "<< i << "ROR = " << std::hex << h_dst[i] << std::endl;
    }
    else{
        std::cout << "Thread: "<< i << "ROL = " << std::hex << h_dst[i] << std::endl;
    }
  }
